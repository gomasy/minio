...
(silo server http://localhost:9000/tmp/xl/{1...10}/disk{0...1} 2>&1 >/tmp/decom.log) &
pid=$!
...
(silo server /tmp/xltier/{1...4}/disk{0...1} --address :9002 2>&1 >/dev/null) &

export MC_HOST_mytier="http://minioadmin:minioadmin@localhost:9002/"

./mc ready mysilo

./mc mb -l mysilo/bucket2
./mc mb -l mytier/tiered

## create a tier and set up ilm policy to tier immediately
./mc admin tier add minio mysilo TIER1 --endpoint http://localhost:9002 --access-key minioadmin --secret-key minioadmin --bucket tiered --prefix prefix5/
./mc ilm add mysilo/bucket2 --transition-days 0 --transition-tier TIER1 --transition-days 0

## mirror some content to bucket2 and capture versions tiered
./mc mirror internal mysilo/bucket2/ --quiet >/dev/null
./mc ls -r mysilo/bucket2/ >bucket2_ns.txt
./mc ls -r --versions mysilo/bucket2/ >bucket2_ns_versions.txt
...
./mc version info mysilo/bucket2 | grep -q "versioning is enabled"
ret=$?
if [ $ret -ne 0 ]; then
...
./mc ls -r mysilo/bucket2 >decommissioned_bucket2_ns.txt
./mc ls -r --versions mysilo/bucket2 >decommissioned_bucket2_ns_versions.txt
...
got_checksum=$(./mc cat mysilo/bucket2/dsync/drwmutex.go | md5sum)
if [ "${expected_checksum}" != "${got_checksum}" ]; then
