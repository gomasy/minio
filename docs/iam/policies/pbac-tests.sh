...
mc admin policy attach mysilo deny-non-sse-kms-pol --user silo123
mc admin policy attach mysilo deny-invalid-sse-kms-pol --user silo123
mc admin policy attach mysilo consoleAdmin --user silo123
...
mc cp /etc/issue mysilo1/multi-key-poc | grep -q "Insufficient permissions to access this path"
ret=$?
if [ $ret -eq 0 ]; then
...
mc cp /etc/hosts mysilo1/multi-key-poc/hosts --enc-kms "mysilo1/multi-key-poc/hosts=minio-default-key"
ret=$?
if [ $ret -ne 0 ]; then
...
mc cp /etc/issue mysilo1/multi-key-poc/issue --enc-kms "mysilo1/multi-key-poc/issue=minio-default-key-xxx" | grep "Insufficient permissions to access this path"
ret=$?
if [ $ret -eq 0 ]; then
